...
const Records = () => {
  ...
  useEffect(() => {
    try{
       axios.get("http://localhost:4000/getRecords",{headers:{
  ...
  useEffect(() => {
    if(Query!=""){
    try{
      axios.post("http://localhost:4000/caseSearch",{Query},{headers:{
         'Content-Type': 'application/json',
         'Access-Control-Allow-Origin': '*'
     }}).then(data=>{
       if(data.status==200){
 ...
 console.log({err});
  ...
  console.log({Data},"-----------");
    ...
    const deleteCase=(id)=>{
      console.log({id});
      try{
        axios.post("http://localhost:4000/deleteCase",{id},{headers:{
           'Content-Type': 'application/json',
           'Access-Control-Allow-Origin': '*'
       }}).then(data=>{
         if(data.status==200){
        ...
        <table className="table table-bordered">
          ...
          <div className="ml-2 d-flex justify-content-center align-items-center " style={{fontSize:"80%"}}>
                  ...
                  <Download download={download} id="3"/>    
                      <EditBtn path={"Dashboard/edit-records"} id={el.id} />
